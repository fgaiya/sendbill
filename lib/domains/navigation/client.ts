...
// アイコン付きサイドバーメニュー項目
export const SIDEBAR_MENU_ITEMS: SidebarMenuItem[] = SIDEBAR_MENU_CONFIG.map(item => ({
  ...item,
  icon: ICON_MAP[item.iconName],
}))

// アイコン付きサイドバーアクション項目
export const SIDEBAR_ACTION_ITEMS: SidebarMenuItem[] = SIDEBAR_ACTION_CONFIG.map(item => ({
  ...item,
  icon: ICON_MAP[item.iconName],
}))