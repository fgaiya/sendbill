...
export async function POST() {
  ...
  if (!BILLING_FLAGS.STRIPE_ENABLED || !secret || !subId) {
    const res = await prisma.company.updateMany({
      where: { id: company.id, stripeSubscriptionId: subId ?? null },
      data: {
        plan: 'FREE',
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const resLocal = await prisma.company.updateMany({
    ...
    const resOk = await prisma.company.updateMany({
      where: { id: company.id, stripeSubscriptionId: subId },
      data: {
        plan: 'FREE',
        subscriptionStatus: 'canceled',
    ...
    if (e instanceof Error && e.name === 'AbortError') {
      const resAbort = await prisma.company.updateMany({
        where: { id: company.id, stripeSubscriptionId: subId ?? null },
        data: {
          plan: 'FREE',
          subscriptionStatus: 'canceled',
    ...
    const resErr = await prisma.company.updateMany({
      where: { id: company.id, stripeSubscriptionId: subId ?? null },
      data: {
        plan: 'FREE',
        subscriptionStatus: 'canceled',
