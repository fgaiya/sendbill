...
import { useQuoteDelete } from '@/lib/domains/quotes/hooks';
...
export function QuoteDeleteButton({
  ...
  const { deleteQuote, isDeleting, deleteError, clearError } = useQuoteDelete();
  ...
  const handleConfirmDelete = async () => {
    const success = await deleteQuote(quote);

    if (success) {
      setShowConfirmDialog(false);

      // 成功トーストを表示
      toast.success(`見積書「${quote.quoteNumber || quote.id}」を削除しました`);

      // カスタムコールバックがある場合は実行、ない場合は見積書一覧にリダイレクト
      if (onDeleteSuccess) {
        onDeleteSuccess();
      } else {
